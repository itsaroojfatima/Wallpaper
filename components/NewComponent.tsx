...
export default function NewComponent() {
  ...
  const fetchNewWallpapers = async () => {
    try {
      setLoading(true);
      // 'abstract design' ya 'minimal aesthetic' query se fresh aur naye wallpapers ache milte hain
      const response = await fetch(
        "https://api.pexels.com/v1/search?query=aesthetic%20minimal&per_page=20",
        {
          headers: {
  ...
  if (loading) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loadingText}>Loading New Wallpapers...</Text>
      </View>
    );
  }
